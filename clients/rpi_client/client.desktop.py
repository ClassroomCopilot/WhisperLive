...
def eos_sensitive_callback(text, start, end, is_final, user):
    global accumulated_text, last_start, last_end
    print(f"Callback received: {text}, Start: {start}, End: {end}, Is final: {is_final}")
    ...
    # Check if the segment is final (eos is True)
    if is_final:
        send_accumulated_text()
        reset_send_timer()
    else:
        # Reset the timer to potentially send the text after a delay if not final
        reset_send_timer()
